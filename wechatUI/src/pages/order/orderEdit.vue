...
<script>
    ...
    export default{
        data(){
            return {
                id: this.$route.params.order_id,    //订单id
        ...
        methods: {
            ...
            async loadData(){
                ...
                if (res.state) {
                    ...
                    self.formData.customer = order.customer_id;
                    self.formData.deliveryDate = new Date(order.deliver_date);
                    self.formData.address = order.delivery_address;
                    self.formData.remark = order.remarks;

                    res.data.goods.forEach(item => {
                        item.sNum = item.num;
                    });
                    self.formData.goods = res.data.goods;
                } else {
                    self.$toast("获取订单信息失败");
                }
            },
            ...
            async submit(){
                ...
                self.$indicator.open();
                let res = await self.$axios.post(self.$api.orderUpdate, data);
                self.$indicator.close();
